...
public class RepositoryManager implements Deserializer, JaxbReferenceResolver {
    ...
    public void register(Object o) {
        ...
        } else if (o instanceof edu.mit.cci.simulation.api.Simulation) {
            edu.mit.cci.simulation.api.Simulation s = (edu.mit.cci.simulation.api.Simulation) o;
            if (!simulationCache.containsKey(s.getIdAsString())) {
                simulationCache.put(s.getIdAsString(), new AdaptedSimulation(s,this));
            }

        } else if (o instanceof edu.mit.cci.simulation.api.Scenario) {
            edu.mit.cci.simulation.api.Scenario s = (edu.mit.cci.simulation.api.Scenario) o;
    ...
    public HasId lookup(String s, Class clazz) {
        HasId result = null;
        if (Simulation.class.isAssignableFrom(clazz) || edu.mit.cci.simulation.api.Simulation.class.isAssignableFrom(clazz)) {
        ...
        } else if (Scenario.class.isAssignableFrom(clazz) || edu.mit.cci.simulation.api.Scenario.class.isAssignableFrom(clazz)) {
    ...
    private Class mapType(String type) {
        if ("Simulation".equals(type)) return edu.mit.cci.simulation.api.Simulation.class;
        else if ("Scenario".equals(type)) return edu.mit.cci.simulation.api.Scenario.class;
    ...
    public HasId getAdaptor(Object o) {
        ...
        if (o instanceof edu.mit.cci.simulation.api.Simulation) {
            return lookup(((edu.mit.cci.simulation.api.Simulation)o).getId()+"",Simulation.class);
        } else if (o instanceof edu.mit.cci.simulation.api.Scenario) {
            return lookup(((edu.mit.cci.simulation.api.Scenario)o).getId()+"",Scenario.class);
