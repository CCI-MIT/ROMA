 ...
 * Time: 12:05 PM
 */
public class AdaptedVariable extends AdaptedObject<edu.mit.cci.simulation.impl.Tuple> implements Variable {


    List<Tuple> values = null;
    edu.mit.cci.simulation.impl.Tuple index = null;



    public AdaptedVariable(edu.mit.cci.simulation.impl.Tuple tuple, Set<edu.mit.cci.simulation.impl.Tuple> peers, RepositoryManager manager) {
        super(tuple,manager);

        if (peers != null && tuple.getVar().getIndexingVariable()!=null) {
            edu.mit.cci.simulation.api.Variable indexing = tuple.getVar().getIndexingVariable();

            for (edu.mit.cci.simulation.impl.Tuple t:peers) {
    ...
    public List<Tuple> getValue() {
        if (values == null) {
            values = new ArrayList<Tuple>();
             edu.mit.cci.simulation.impl.Tuple[] inputs = null;
            if (index!=null) {
                inputs = new edu.mit.cci.simulation.impl.Tuple[] {index,model()};
            } else {
                inputs = new edu.mit.cci.simulation.impl.Tuple[] {model()};
            }
