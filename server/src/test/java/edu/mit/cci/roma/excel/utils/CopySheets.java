...
public class CopySheets {
	...
	public static Workbook mergeExcelFiles(Workbook book,
			File[] inList) throws IOException, InvalidFormatException {

		for (File fileIn : inList) {
			Workbook b = WorkbookFactory.create(fileIn);
			for (int i = 0; i < b.getNumberOfSheets(); i++) {
				// not entering sheet name, because of duplicated names
				System.out.println("Copying sheet: " + b.getSheetAt(i).getSheetName());
				copySheets(book.createSheet(fileIn.getName() + b.getSheetAt(i).getSheetName()), b.getSheetAt(i));
			}
		}
		return book;
	}
